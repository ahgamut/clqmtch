...
public class ParallelStackDFS {
  ...
  public void process_graph(Graph G, int lower_bound, int upper_bound) {
    ...
    process_vertex(G, G.CUR_MAX_CLIQUE_LOCATION, lock);
    IntStream.range(0, G.n_vert - 1)
            .parallel()
            .unordered()
            .forEach(z -> process_vertex(G, (G.n_vert - 1) - z, lock));
  }

  private void process_vertex(Graph G, int cur, ReentrantLock lock) {
    int candidates_left;
    int clique_size;
    ...
    if (vcur.mcs <= G.CUR_MAX_CLIQUE_SIZE) {
      return;
    }
      ...
      for (j = cur_state.start_at; j >= 0 && j < vcur.N; j = cur_state.start_at) {
        ...
        if (clique_potential > G.CUR_MAX_CLIQUE_SIZE) {
          ...
          if (candidates_left == 0) {
            ...
            lock.lock();
            if(clique_potential > G.CUR_MAX_CLIQUE_SIZE) {
              // System.out.printf("%d < %d, %d %d\n",G.CUR_MAX_CLIQUE_SIZE, clique_potential, cur,lock.getQueueLength());
